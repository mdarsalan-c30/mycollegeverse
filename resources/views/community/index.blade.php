                        ...
                        <div class="flex">
                            ...
                            <div class="flex-1 p-6 lg:p-8">
                                ...
                                <div class="flex items-center gap-6 pt-6 border-t border-slate-50">
                                    <button onclick="togglePostComments({{ $post->id }})" class="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-primary transition-colors uppercase tracking-widest">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                                        {{ $post->comments_count }} Comments
                                    </button>
                                    <button onclick="handleShare('{{ route('community.index') }}#post-{{ $post->id }}')" class="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-primary transition-colors uppercase tracking-widest group/share">

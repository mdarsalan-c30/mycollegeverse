        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div x-show="!mode" class="max-w-4xl mx-auto text-center py-20 animate-fade-in">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <button @click="mode = 'guided'" class="bg-white p-10 rounded-[3rem] shadow-xl group text-left">
                        <h3 class="text-2xl font-black uppercase italic mb-2">Guided Builder</h3>
                        <p class="text-slate-400 font-bold text-sm">Fast, role-based auto-fill.</p>
                    </button>
                    <button @click="mode = 'latex'" class="bg-slate-900 p-10 rounded-[3rem] shadow-xl group text-left">
                        ...
                        <p class="text-slate-400 font-bold text-sm">Full control via LaTeX code.</p>
                    </button>
                </div>
            </div>
            ...
            <div x-show="mode === 'latex'" style="display: none;" class="animate-fade-in h-[85vh]">
                <div class="flex items-center justify-between mb-8">
                    <button @click="mode = null" class="text-slate-400 font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:text-primary">Back</button>
                    <div class="flex items-center gap-4">
                        ...
                        <button @click="saveResume" class="bg-primary text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl">Save & Manifest</button>
                    </div>
                </div>
            ...
            <!-- Guided Mode (Omitting but keep functionality) -->
        </div>
    </div>
        ...
        function resumeBuilder() {
            return {
                ...
                recompile() {
                    ...
                    let match;
                    while ((match = newCommandRegex.exec(code)) !== null) {
                    ...
                    const role = code.match(/\\small ([^}]+)\}/)?.[1] || "Student Profile";
                    
                    html += `<div class="flex justify-between items-start mb-6 border-b-2 border-black pb-4">
                        <div><h1 class="text-2xl font-bold uppercase">${name}</h1><p class="text-xs font-bold text-slate-600 uppercase">${role}</p></div>
                    ...
                    if (sections) {
                        sections.forEach(sec => {
                            ...
                            if (itemizeMatch) {
                                ...
                                if (items) {
                                    html += `<div><h3 class="text-xs font-black uppercase border-b border-black pb-1 mb-2">${title}</h3>`;

    ...
    <div class="min-h-screen bg-slate-50 py-12" x-data="resumeBuilder()" x-init="init()">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div x-show="mode === 'guided'" style="display: none;" class="animate-fade-in">
                ...
                <!-- Editor ... (Omitting rest for brevity but keep the logic same) -->
                <div x-show="roleSelected" class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                     <div class="space-y-8">
                        ...
                        <div class="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-slate-100 min-h-[600px]">
                            ...
                            <!-- Projects, Skills etc would go here -->
                             <div x-show="currentStep === 1" class="space-y-8 animate-fade-in">
                                <div class="flex justify-between items-center"><h2 class="text-2xl font-black text-slate-800 uppercase italic">Projects</h2><button @click="addProject" class="text-primary font-black uppercase text-[10px]">+ Add</button></div>
                                <template x-for="(proj, index) in resume.projects" :key="index">
                                    <div class="bg-slate-50 p-6 rounded-3xl mb-4">
                                        <input type="text" x-model="proj.title" placeholder="Title" class="w-full bg-transparent border-none font-black text-slate-800 p-0 mb-2">
                            ...
                            <div x-show="currentStep === 2" class="space-y-8 animate-fade-in">
                                <h2 class="text-2xl font-black text-slate-800 uppercase italic">Skills</h2>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <template x-for="(skill, index) in resume.skills" :key="index"><span class="bg-slate-100 px-4 py-2 rounded-xl text-xs font-bold text-slate-600 flex items-center gap-2"><span x-text="skill"></span><button @click="removeSkill(index)">×</button></span></template>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    if (sections) {
                        sections.forEach(sec => {
                            ...
                            content = content.replace(/\\begin\{itemize\}[\s\S]*?\\end\{itemize\}/g, (list) => {
                                ...
                                if (!items) return '';
                                return `<ul class="list-disc ml-4 mt-1">${items.map(i => `<li class="text-[11px] mb-0.5">${i.replace('\\item ', '').trim()}</li>`).join('')}</ul>`;
                            });

                            html += `<div><h3 class="text-xs font-black uppercase border-b border-black pb-1 mb-2">${title}</h3><div class="text-[11px] text-justify leading-snug">${content}</div></div>`;
                        });
                    }
                    html += '</div>';

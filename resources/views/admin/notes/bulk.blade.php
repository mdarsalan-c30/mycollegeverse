...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-800 tracking-tight">AI Staging Area <span class="text-xs bg-emerald-100 text-emerald-600 px-3 py-1 rounded-full uppercase ml-2 tracking-tighter">Phase 4 Stable</span></h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2" x-show="!showStaging" x-transition>
            <div class="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-8">
                ...
                {{-- Step 2: Subject Selector (Optimized) --}}
                <div class="space-y-3">
                    ...
                    <select x-model="selectedSubject" required
                            class="w-full h-16 bg-slate-50 border-slate-100 rounded-2xl px-6 text-sm font-bold text-slate-800">
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-6">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                            Review and Manifest individual knowledge assets
                        </p>
                    </div>
                    <button type="button" @click="generateSelected()" 
                            :disabled="isGenerating || topicsList.filter(i => i.selected && i.status === 'pending').length === 0"
                            class="bg-primary text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2">
                        <svg x-show="isGenerating" class="animate-spin h-3 w-3 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                        Launch Selected
                    </button>
                </div>
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-2xl border transition-all"
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <template x-if="item.status === 'pending'">
                                    <button type="button" :disabled="isGenerating" @click="generateItem(item)" class="bg-white text-slate-600 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase border border-slate-200 hover:bg-primary hover:text-white hover:border-primary transition-all">
                                        Generate
                                    </button>
                                </template>
                            </div>
        ...
        <div class="space-y-6">
            <div class="bg-violet-600 p-8 rounded-[2rem] text-white space-y-4">
                ...
                <h3 class="text-xl font-bold">Ultra-Lite Mode</h3>
                <p class="text-xs text-violet-100 leading-relaxed font-medium">
                    This page has been optimized to bypass Hostinger database connection limits.
                </p>
                <div class="space-y-4 pt-4 border-t border-white/10">
                    <div class="flex gap-3">
                        <div class="w-2 h-2 bg-emerald-400 rounded-full shrink-0 animate-pulse mt-1"></div>
                        <p class="text-[10px] font-bold text-violet-100">Zero-Load Index: Analytics counting disabled for maximum speed.</p>
                    </div>
                    <div class="flex gap-3">
                        <div class="w-2 h-2 bg-emerald-400 rounded-full shrink-0 animate-pulse mt-1"></div>
                        <p class="text-[10px] font-bold text-violet-100">Atomic Generation: Process one topic at a time to prevent server timeout.</p>
                    </div>
                </div>
            </div>

            <div class="bg-white p-6 rounded-[2rem] border border-slate-100 space-y-4">
                <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">System Check</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-xs font-bold">
                        <span class="text-slate-500">AI Core</span>
                        <span class="text-slate-800">Phase 4 Active</span>
                    </div>
                </div>
            </div>
        </div>

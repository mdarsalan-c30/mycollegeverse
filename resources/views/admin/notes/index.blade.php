<x-admin-layout>
    <div class="space-y-8" x-data="{ 
        ...
        <div class="flex items-center gap-2 border-b border-slate-100 pb-px">
            <a href="{{ route('admin.notes', ['status' => 'pending']) }}" class="px-6 py-3 text-[10px] font-black uppercase tracking-widest transition-all {{ request('status') == 'pending' ? 'text-admin-primary border-b-2 border-admin-primary' : 'text-slate-400 hover:text-slate-600' }}">Pending Verification</a>
            ...
            <a href="{{ route('admin.notes', ['status' => 'all']) }}" class="px-6 py-3 text-[10px] font-black uppercase tracking-widest transition-all {{ (!request('status') || request('status') == 'all') ? 'text-slate-600 border-b-2 border-slate-600' : 'text-slate-400 hover:text-slate-600' }}">All Uploads</a>
        </div>
            ...
            <div class="glass p-8 rounded-[2.5rem] border-white/50 shadow-xl shadow-slate-200/50 group hover:bg-white transition-all flex flex-col items-start italic">
                ...
                <div class="w-full mt-8 pt-8 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <!-- Reject Action (Always available to Master Authority) -->

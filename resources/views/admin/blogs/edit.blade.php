...
@section('content')
<form action="{{ route('admin.blogs.update', $blog) }}" method="POST" class="space-y-10" x-data="{ 
    autoRecommend: {{ $blog->auto_recommend_colleges ? 'true' : 'false' }},
    title: '{{ addslashes($blog->title) }}',
    metaDesc: '{{ addslashes($blog->meta_description) }}',
    keywords: '{{ addslashes($blog->meta_keywords) }}',
    ...
    updateSeo() {
        ...
        if(this.keywords.split(',').length >= 3) score += 25;
        if(this.title.length > 5) score += 25;
        this.seoScore = score;
    }
}">
    @csrf
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
            <h1 class="text-4xl font-black text-secondary tracking-tight">Reconfigure Article</h1>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 italic">Optimizing Knowledge for the Multiverse Index</p>
        </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="glass p-8 rounded-[2.5rem] border-white/60 shadow-sm space-y-6">
                <div>
                    ...
                    <input type="text" name="title" x-model="title" @input="updateSeo()" required 
                ...
                <div>
                    ...
                    <textarea name="content" id="article-editor" required class="w-full bg-white border-slate-100 rounded-2xl p-6 text-sm font-medium leading-relaxed min-h-[500px] focus:ring-4 focus:ring-admin-primary/5 focus:border-admin-primary transition-all">{{ $blog->content }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="3" class="w-full bg-white border-slate-100 rounded-2xl p-6 text-xs font-medium focus:ring-4 focus:ring-admin-primary/5" placeholder="Optional: Summarize the article for feed previews...">{{ $blog->excerpt }}</textarea>
                </div>
            </div>
            ...
            <div class="glass p-8 rounded-[2.5rem] border-white/60 shadow-sm space-y-6">
                ...
                <div x-show="!autoRecommend" x-transition class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto custom-scrollbar pr-2 p-1">
                        ...
                        <label class="p-4 border-2 {{ in_array($college->id, $blog->college_ids ?? []) ? 'border-admin-primary/30 bg-admin-primary/5' : 'border-slate-50' }} rounded-2xl flex items-center gap-4 cursor-pointer hover:border-admin-primary/20 hover:bg-slate-50 transition-all h-16">
                            <input type="checkbox" name="college_ids[]" value="{{ $college->id }}" {{ in_array($college->id, $blog->college_ids ?? []) ? 'checked' : '' }} class="w-5 h-5 rounded-lg text-admin-primary focus:ring-admin-primary/20 bg-white border-slate-200">
                            <div class="min-w-0">
        ...
        <div class="space-y-8">
            ...
            <div class="glass p-8 rounded-[2.5rem] border-white/60 shadow-sm space-y-6 overflow-hidden relative">
                ...
                <div class="flex items-end gap-3 mb-6 relative">
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Current Health</p>
                </div>
                
                <div class="space-y-6 relative">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Meta Title</label>
                        <input type="text" name="meta_title" value="{{ $blog->meta_title }}" class="w-full h-12 bg-white border-slate-100 rounded-xl px-4 text-xs font-bold focus:ring-4 focus:ring-admin-primary/5">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="meta_keywords" x-model="keywords" @input="updateSeo()"
                    ...
                    <div>
                        ...
                        <input type="number" name="ai_score" value="{{ $blog->ai_score }}" class="w-full h-12 bg-white border-slate-100 rounded-xl px-4 text-xs font-bold focus:ring-4 focus:ring-admin-primary/5">
                    </div>
                </div>
            </div>
            ...
            <div class="glass p-8 rounded-[2.5rem] border-white/60 shadow-sm space-y-6">
                ...
                <div class="space-y-4">
                    <label class="p-4 border-2 {{ $blog->is_published ? 'border-emerald-100 bg-emerald-50/20 text-emerald-600' : 'border-slate-50 text-slate-400' }} rounded-2xl flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-all">
                        <span class="text-xs font-black tracking-tight">Public Presence (Live)</span>
                        <input type="checkbox" name="is_published" value="1" {{ $blog->is_published ? 'checked' : '' }} class="w-6 h-6 rounded-lg text-emerald-500 border-slate-200 focus:ring-emerald-500/20 shadow-sm">
                    </label>
                    ...
                    <div>
                        ...
                        <input type="text" name="featured_image" value="{{ $blog->featured_image }}" placeholder="ImageKit Path or Remote URL..." 
                               class="w-full h-12 bg-white border-slate-100 rounded-xl px-4 text-xs font-bold focus:ring-4 focus:ring-admin-primary/5">
                    </div>
                </div>
            </div>

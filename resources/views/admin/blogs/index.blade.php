<x-admin-layout>
    ...
    <div class="space-y-10">
        ...
        @if(session('error'))
        <div class="p-6 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-4 text-rose-600 animate-pulse">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 class="font-black text-secondary text-sm group-hover:text-admin-primary transition-colors line-clamp-1 max-w-[200px]">{{ $blog->title }}</h4>
                                    <p class="text-[10px] font-bold text-slate-400 mt-1 italic">{{ optional($blog->author)->name ?? 'Archon' }} · {{ optional($blog->created_at)->format('M d, Y') ?? 'Eternal' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">

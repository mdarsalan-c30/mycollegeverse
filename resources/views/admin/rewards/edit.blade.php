<x-admin-layout>
    @section('title', 'Refine Perk | Control Tower')

    <div class="max-w-4xl mx-auto space-y-10">
        <!-- Header Node -->
        <div>
            <h1 class="text-4xl font-black text-secondary tracking-tight">Refine Perk</h1>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 italic">Reconfiguring Reward Asset: {{ $reward->title }}</p>
        </div>

        <form action="{{ route('admin.rewards.update', $reward) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="glass p-10 rounded-[2.5rem] border-white/60 shadow-sm space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title" required value="{{ old('title', $reward->title) }}" 
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="number" name="karma_required" required value="{{ old('karma_required', $reward->karma_required) }}"
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" required rows="3"
                        class="w-full bg-slate-50 border-none rounded-3xl px-6 py-4 text-sm font-bold text-secondary focus:ring-2 focus:ring-admin-primary/20 placeholder:text-slate-300 transition-all">{{ old('description', $reward->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="number" name="max_usage" required value="{{ old('max_usage', $reward->max_usage) }}"
                            class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold text-secondary focus:ring-2 focus:ring-admin-primary/20 transition-all">
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="datetime-local" name="expires_at" value="{{ old('expires_at', $reward->expires_at ? $reward->expires_at->format('Y-m-d\TH:i') : '') }}"
                            class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold text-secondary focus:ring-2 focus:ring-admin-primary/20 transition-all">
                    </div>
                </div>
                ...
                <!-- Claim Link -->
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Redemption Nexus URL</label>
                    <div class="relative">
                        <span class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
                        </span>
                        <input type="url" name="claim_link" required value="{{ old('claim_link', $reward->claim_link) }}"
                ...
                <div class="flex items-center gap-3 ml-1">
                    ...
                    <label for="is_active" class="text-xs font-black text-slate-500 uppercase tracking-widest">Active Pool Presence</label>
                </div>
            </div>

            <div class="flex items-center justify-end gap-4">
                <a href="{{ route('admin.rewards.index') }}" class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-secondary transition-colors">Abort Refinement</a>

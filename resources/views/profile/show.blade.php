                ...
                <div class="glass p-8 rounded-[3rem] shadow-glass border-white/40 flex flex-col md:flex-row items-center md:items-end gap-8">
                    ...
                    <div class="flex-1 text-center md:text-left space-y-2 pb-2">
                        <div class="flex flex-wrap items-center justify-center md:justify-start gap-4">
                            <h2 class="text-3xl md:text-4xl font-black text-secondary tracking-tighter">{{ $user->name }}</h2>
                            @if($user->karma >= 1000)
                            <span class="bg-amber-100/80 text-amber-700 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-amber-200">Tier-1 Signal</span>
                            @endif
                        </div>
                        <p class="text-slate-500 font-bold max-w-lg">{{ $user->career_role ?? 'Academic Curator' }} • {{ $user->college->name ?? 'Campus Node' }}</p>
                        
                        <!-- Social Connectors -->
                        <div class="flex items-center justify-center md:justify-start gap-4 pt-2">
                            @if(isset($socialLinks['linkedin']))
                            <a href="{{ $socialLinks['linkedin'] }}" target="_blank" class="w-8 h-8 rounded-xl bg-[#0077b5]/10 text-[#0077b5] flex items-center justify-center hover:scale-110 transition-transform">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                            </a>
                            @endif
                            @if(isset($socialLinks['github']))
                            <a href="{{ $socialLinks['github'] }}" target="_blank" class="w-8 h-8 rounded-xl bg-slate-900/10 text-slate-900 flex items-center justify-center hover:scale-110 transition-transform">
                            ...
                            <a href="{{ $socialLinks['behance'] }}" target="_blank" class="w-8 h-8 rounded-xl bg-[#1769ff]/10 text-[#1769ff] flex items-center justify-center hover:scale-110 transition-transform">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M22 7h-7v-2h7v2zm-11.5 2.5c-1.4 0-2.5 1.1-2.5 2.5s1.1 2.5 2.5 2.5 2.5-1.1 2.5-2.5-1.1-2.5-2.5-2.5zm-4.5 4.5c0 1.4-1.1 2.5-2.5 2.5s-2.5-1.1-2.5-2.5 1.1-2.5 2.5-2.5 2.5 1.1 2.5 2.5zm0-4c0 1.4-1.1 2.5-2.5 2.5s-2.5-1.1-2.5-2.5 1.1-2.5 2.5-2.5 2.5 1.1 2.5 2.5zm13.5 4c0 1.4-1.1 2.5-2.5 2.5s-2.5-1.1-2.5-2.5 1.1-2.5 2.5-2.5 2.5 1.1 2.5 2.5z"/></svg>
                            </a>
                            @endif
                            @if($isOwner)
                            <button @click="editMode = true" class="text-[10px] font-black text-primary uppercase tracking-widest hover:underline ml-2">+ Connect socials</button>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4 md:pb-4">
                        @if($isOwner)
                        <button @click="editMode = true" class="bg-primary text-white px-8 py-4 rounded-[1.5rem] font-bold shadow-xl shadow-primary/20 hover:scale-105 transition-all text-sm">Professional Sync</button>
                        @else
                        <a href="{{ route('chat.index', $user->username) }}" class="bg-primary text-white px-8 py-4 rounded-[1.5rem] font-bold shadow-xl shadow-primary/20 hover:scale-105 transition-all text-sm flex items-center gap-2">
                            <span>Connect</span>
        ...
        <div class="pt-20 grid lg:grid-cols-3 gap-10" x-data="{ activeTab: 'projects' }">
            ...
            <div class="space-y-8">
                ...
                <div class="glass p-8 rounded-[2.5rem] shadow-glass border-white/60">
                    <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-8 border-b border-slate-100 pb-4">Professional Identity</h4>
                    <div class="space-y-8">
